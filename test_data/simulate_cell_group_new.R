...
params <- newSplatParams(batchCells = 25000, nGenes = 5000, 
                         group.prob = c(0.09,0.09,0.1,0.08,0.06,0.11,0.1,0.06,0.1,0.06,0.05,0.1),
                         #dropout.type = "experiment",
                         dropout.mid = 0,
                         dropout.shape = -1,
                         de.prob = 0.3,
                         de.facScale = c(0.3,0.3,0.3,0.3,0.3,0.3,0.3,0.3,0.3,0.3,0.3,0.3),
                         seed = 8)
...
write.table(sim4@assays$SCT@scale.data, file='data3_dropout_SCT_0.csv', quote=FALSE, sep='\t', col.names = TRUE, row.names = TRUE)

write.table(t(simcounts), file='data3_raw.csv', quote=FALSE, sep='\t', col.names = NA)

write.table(simcounts, file='data3_dropout_raw_0.csv', quote=FALSE, sep=',', col.names = TRUE, row.names = TRUE)

normalized_data <- sctransform::vst(simcounts)$y
write.table(idents, file='idents0_SCT2.csv', quote=FALSE, sep='\t', col.names = NA)
write.table(t(normalized_data), file='data0_SCT2.csv', quote=FALSE, sep='\t', col.names = NA)
